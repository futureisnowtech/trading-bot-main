...
*   **GraphCast AI (AIGEFS)**: Google DeepMind's machine-learning weather forecasting model. Used in this bot as a volatility ($\sigma$) consensus filter.
...
*   **Sovereign SRE Ceilings**: SRE-enforced risk limits (such as the `$40.00` USD absolute budget ceiling per position or a `2500` contract maximum).
