...
_MAX_SPREAD_PCT          = 0.25      # relaxed for Kraken
...
_SETUP_PRIORITY = ['supertrend', 'kst_cross', 'momentum', 'ranging_mr', 'funding_collect']
...
def _kraken_klines(symbol: str, interval: str, n_bars: int = 60) -> List[List[float]]:
    ...
    if interval not in _KRAKEN_INTERVALS:
        return []
    bar_secs = {'1m': 60, '5m': 300, '15m': 900, '30m': 1800,
                '1h': 3600, '4h': 14400, '1d': 86400, '1w': 604800}.get(interval, 900)
    ...
    for k in data.get('candles', []):
        try:
            rows.append([float(k['open']), float(k['high']), float(k['low']),
...
def _binance_tickers() -> List[Dict]:
    ...
    if data is None:
        logger.info('[scanner] Binance tickers unavailable (geo-block or down) — Kraken only')
        return []
...
def _binance_klines(symbol: str, interval: str, n_bars: int = 60) -> List[List[float]]:
    ...
    for k in data:
        try:
            rows.append([float(k[1]), float(k[2]), float(k[3]),
                         float(k[4]), float(k[7])])  # OHLC + quote volume (USDT)
        except Exception:
            continue
    return rows  # ascending (oldest first)
...
def _calc_adx(highs: List[float], lows: List[float], closes: List[float],
    ...
    dm_p = np.where((h[1:] - h[:-1]) > (lo[:-1] - lo[1:]), np.maximum(h[1:] - h[:-1], 0.0), 0.0)
    dm_m = np.where((lo[:-1] - lo[1:]) > (h[1:] - h[:-1]), np.maximum(lo[:-1] - lo[1:], 0.0), 0.0)
    def _smooth(a, p):
        s = np.zeros(len(a))
        if len(a) < p: return s
        s[p-1] = a[:p].sum()
...
def _calc_vol_spike(volumes: List[float], window: int = 20) -> float:
    ...
    if len(volumes) < window + 1:
        return 1.0
    cur = volumes[-1]
    avg = (float(np.mean(volumes[-window-1:-1])) if _NUMPY_OK
...
def _calc_kst(closes: List[float]) -> Tuple[float, float, bool, bool]:
    ...
    for roc_p, sma_p in zip(ROC_P, SMA_P):
        roc = [(c[i] - c[i-roc_p]) / (c[i-roc_p] + 1e-9) * 100
               for i in range(roc_p, len(c))]
        if len(roc) < sma_p:
            return 0.0, 0.0, False, False
        sma = [sum(roc[i-sma_p+1:i+1]) / sma_p for i in range(sma_p-1, len(roc))]
        rcmas.append(sma)
    ...
    kst_s = [sum(w * rcmas[j][len(rcmas[j]) - min_len + i]
                 for j, w in enumerate(WEIGHTS))
             for i in range(min_len)]
    sig_s = [sum(kst_s[i-SIG_P+1:i+1]) / SIG_P
             for i in range(SIG_P-1, len(kst_s))]
...
def _calc_supertrend(highs: List[float], lows: List[float], closes: List[float],
                     period: int = 10, mult: float = 3.0) -> Tuple[int, bool, bool]:
    """
    ...
    for i in range(1, len(c)):
        ...
        if   dirs[i-1] == -1 and c[i] > fub[i]: dirs[i] =  1
        elif dirs[i-1] ==  1 and c[i] < flb[i]: dirs[i] = -1
        else:                                     dirs[i] = dirs[i-1]
    cur, prv = dirs[-1], dirs[-2]
    return cur, (cur == 1 and prv == -1), (cur == -1 and prv == 1)


def _calc_vwap(highs: List[float], lows: List[float],
               closes: List[float], volumes: List[float]) -> float:
    """Session VWAP from provided bars."""
    total_vol = sum(volumes)
    if total_vol <= 0:
        return closes[-1] if closes else 0.0
    return sum((h + lo + c) / 3 * v for h, lo, c, v in zip(highs, lows, closes, volumes)) / total_vol
...
def _step1_universe(kraken_tickers: List[Dict],
                    binance_tickers: List[Dict],
                    binance_funding: Dict[str, float]) -> List[Dict]:
    """
    ...
    for t in kraken_tickers:
        sym = t.get('symbol', '')
        if not sym.startswith('PF_'): continue
        if t.get('tag') != 'perpetual': continue
        if t.get('suspended', False): continue
        try:
    ...
    for t in binance_tickers:
        ...
        if not base or any(ch.isdigit() for ch in base): continue
        try:
    ...
    logger.info(f'[scanner] Step 1 (vol>${_MIN_VOLUME_24H_USD/1e3:.0f}K): '
...
def _evaluate_one_symbol(c: Dict) -> List[Dict]:
    ...
    try:
        ...
        if len(vols) >= 2 and vols[-2] > 0 and vols[-1] / vols[-2] < 0.10:
            ...
            lows  = lows[:-1];  closes = closes[:-1]; vols = vols[:-1]
        ...
        if activity and adx >= _MIN_ADX_MOMENTUM:
            direction = 'LONG' if (closes[-1] > closes[-min(4, len(closes)-1)]) else 'SHORT'
        ...
        for direction, setups in fired.items():
            ...
            candidate = {**c,  # carry forward exchange, base_asset, funding_rate, etc.
                'direction':         direction,
...
def _check_ob_one(c: Dict) -> Optional[Dict]:
    ...
    try:
        ...
        if (bid_depth >= _MIN_OB_DEPTH_USD and ask_depth >= _MIN_OB_DEPTH_USD
    ...
    for c in candidates:
        try:
            ...
            if ev >= _MIN_EXPECTED_PROFIT:
                ...
                logger.debug(f'[scanner] step4 {c["symbol"]} {c["direction"]}: '
...
def _step7_rank_and_top(candidates: List[Dict], n: int = _TOP_N) -> List[Dict]:
    ...
    for c in candidates:
        key = (c.get('base_asset', c['symbol']), c['direction'], c.get('primary_setup', ''))
        existing = best.get(key)
        if existing is None or c.get('expected_profit', 0) > existing.get('expected_profit', 0):
            best[key] = c

    ranked = sorted(best.values(), key=lambda x: x.get('expected_profit', 0), reverse=True)
...
def scan(open_positions: Optional[List[str]] = None,
    ...
    try:
        ...
        logger.info(f'[scanner] Step 4 (EV≥${_MIN_EXPECTED_PROFIT}): '
