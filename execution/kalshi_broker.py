...
class KalshiBroker:
    ...
    def _try_maker_exit(
        ...
        *,
        ...
        """Rest a post-only sell at the ask; return a fill result, or None to cross.

        Kalshi refuses reduce_only on a resting order ("reduce_only can only be
        used with IoC orders"), so this order goes out without it. reduce_only
        guaranteed we never sell more than we hold and never open a short; that
        guarantee is rebuilt here:

          * qty is capped at the live broker position before placing,
          * the position is re-read every MAKER_EXIT_POLL_S and the order pulled
            the moment it drops below what is still outstanding.

        Residual exposure is one poll interval, and only if another path flattens
        the position while we rest. Every failure returns None and crosses.
        """
        timeout_s = max(0, int(getattr(config, "MAKER_EXIT_TIMEOUT_S", 60)))
        poll_s = max(0.5, float(getattr(config, "MAKER_EXIT_POLL_S", 2.0)))

        held = self.live_position_qty(ticker)
        if held < 0:
            logger.info("[MakerExit] Position unknown for %s; crossing instead.", ticker)
            return None
        rest_qty = int(min(int(qty), int(held)))
        if rest_qty <= 0:
            logger.info(
                "[MakerExit] Hold %.2f < requested %d on %s; crossing instead.",
                held, qty, ticker,
            )
            return None
        ...
        try:
            ...
            logger.warning("[MakerExit] Quote failed for %s: %s", ticker, exc)
            return None
        raw_ask = quote.get("yes_ask") if side == "yes" else quote.get("no_ask")
        try:
            ask = float(raw_ask or 0.0)
        except (TypeError, ValueError):
            ask = 0.0
        if ask <= 0.0:
            logger.info("[MakerExit] No ask on %s; crossing instead.", ticker)
            return None
        if ask <= float(taker_limit_price):
            logger.info(
                "[MakerExit] Ask %.4f <= limit %.4f on %s; crossing instead.",
                ask, float(taker_limit_price), ticker,
            )
            return None

        body = self._build_event_order_body(
            ticker=ticker, right=right, qty=rest_qty, limit_price=ask,
            action="sell", reduce_only=False, post_only=True,
        )
        resp = self._request("POST", "/trade-api/v2/portfolio/events/orders", body=body)
        error_code = self._extract_error_code(resp)
        if error_code:
            logger.info("[MakerExit] Post-only rejected on %s (%s); crossing.", ticker, error_code)
            return None

        order_info, _status = self._normalize_order_response(resp, requested_qty=rest_qty)
        order_id = str(order_info.get("order_id") or "").strip()
        if not order_id:
            return None

        logger.info(
            "[MakerExit] Resting sell %s x%d @ %.4f (taker would be %.4f), waiting %ds",
            ticker, rest_qty, ask, float(taker_limit_price), timeout_s,
        )
        ...
        try:
            while time.time() < deadline:
                ...
                if filled >= rest_qty:
                    break
                still_held = self.live_position_qty(ticker)
                outstanding = rest_qty - filled
                if 0 <= still_held < outstanding:
                    aborted = (
                        f"position fell to {still_held:.2f} below outstanding {outstanding:.2f}"
                    )
                    break
        except Exception:
            self.cancel_order(order_id)
            logger.exception("[MakerExit] Error while waiting on %s; cancelled and crossing.", ticker)
            return None

        logger.info(
            "[MakerFill] leg=exit ticker=%s qty=%d rest_price=%.4f taker_price=%.4f "
            "filled=%.2f waited_s=%.1f time_to_first_fill_s=%s timeout_s=%d outcome=%s%s",
            ticker, rest_qty, ask, float(taker_limit_price), filled,
            time.time() - rested_at,
            f"{first_fill_at - rested_at:.1f}" if first_fill_at else "none",
            timeout_s,
            "full" if filled >= rest_qty else ("partial" if filled > 0 else "none"),
            f" abort={aborted}" if aborted else "",
        )

        if aborted:
            self.cancel_order(order_id)
            logger.warning("[MakerExit] Pulled resting sell on %s: %s.", ticker, aborted)
            if filled <= 0.0:
                return None
        elif filled <= 0.0:
            self.cancel_order(order_id)
            logger.info("[MakerExit] No fill on %s within %ds; crossing.", ticker, timeout_s)
            return None
        elif filled < rest_qty:
            self.cancel_order(order_id)
            logger.info(
                "[MakerExit] Partial fill %.0f/%d on %s; keeping maker fill.",
                filled, rest_qty, ticker,
            )
        ...
        result["status"] = "executed" if filled >= rest_qty else "partial"
        return result
    ...
    def place_sell_order(self, contract_dict: dict, qty: int, limit_price: float, **kwargs) -> dict:
        ...
        # Maker-first exit, discretionary exits only. Risk-driven exits (salvage,
        # admissibility, stop) always cross: the point of those is to be flat now.
        exit_reason = str(kwargs.get("reason") or "")
        if (
            config.MAKER_EXIT_ENABLED
            and order_type == "limit"
            and any(r and r in exit_reason for r in config.MAKER_EXIT_ELIGIBLE_REASONS)
        ):
            maker_result = self._try_maker_exit(
                contract_dict=contract_dict, ticker=ticker, right=right, side=side,
                qty=qty, taker_limit_price=limit_price, kwargs=kwargs,
            )
            if maker_result is not None:
                return maker_result
