...
def build_features(
    ...
    try:
        ob = _ind('orderbook') or get_orderbook_signal(symbol, float(df['close'].iloc[-1]), feeds)
    except Exception:
        ob = {}
    ...
    result['price_vs_session_vwap_pct'] = float(np.clip(vwap.get('session_vwap_dist_pct', 0.0) / 10, -1, 1))
    result['price_vs_weekly_vwap_pct']  = float(np.clip(vwap.get('weekly_vwap_dist_pct', 0.0) / 10, -1, 1))
    ...
    result['vol_dollar_normalized'] = float(np.clip(of.get('dollar_volume_normalized', 1.0), 0, 10))
    result['vol_trend_slope']       = float(np.clip(of.get('volume_trend_slope', 0.0), -5, 5))
    result['vol_at_price_pct']      = float(np.clip(of.get('volume_at_price_level_pct', 0.0), 0, 1))

    # ── cvd (5) ────────────────────────────────────────────────────────────
    result['cvd_value_normalized'] = float(np.clip(cvd.get('cvd_value_normalized', 0.0), -5, 5))
    ...
    # ── momentum (7) ───────────────────────────────────────────────────────
    result['mom_macd_hist_fast']    = float(np.clip(macd.get('macd_hist_3_15_3', 0.0) / (current + 1e-9) * 100, -5, 5))
    result['mom_macd_hist_slow']    = float(np.clip(macd.get('macd_hist_6_20_5', 0.0) / (current + 1e-9) * 100, -5, 5))
    result['mom_rsi_14']            = float(np.clip(rsi.get('rsi_14', 50.0) / 100, 0, 1))
    result['mom_rsi_21']            = float(np.clip(rsi.get('rsi_21', 50.0) / 100, 0, 1))
    result['mom_williams_r']        = float(np.clip((wr.get('williams_r_14', -50.0) + 100) / 100, 0, 1))
    result['mom_macd_long_aligned'] = 1.0 if macd.get('macd_long_aligned', False) else 0.0
    rsi_div = rsi.get('rsi_divergence_type', 'none')
    result['mom_rsi_divergence']    = float(_DIV_MAP.get(rsi_div, 0.0))

    # ── vwap (4) ───────────────────────────────────────────────────────────
    result['vwap_session_dist_pct'] = float(np.clip(vwap.get('session_vwap_dist_pct', 0.0) / 5, -1, 1))
    result['vwap_weekly_dist_pct']  = float(np.clip(vwap.get('weekly_vwap_dist_pct', 0.0) / 5, -1, 1))
    result['vwap_band_position']    = float(np.clip(vwap.get('vwap_band_position', 0) / 3, -1, 1))
    ...
    result['ob_wall_above_pct'] = float(np.clip(wall_above / 10, 0, 1))   # normalize 0-10%
    result['ob_wall_below_pct'] = float(np.clip(wall_below / 10, 0, 1))

    # ── derivatives (6) ────────────────────────────────────────────────────
    result['deriv_funding_rate']  = float(np.clip(fr.get('funding_rate_current', 0.0) / 0.002, -1, 1))
    result['deriv_carry_annual']  = float(np.clip(fr.get('carry_annual', 0.0) / 100, -1, 1))
    oi_sig = oi.get('oi_signal', 'neutral')
    result['deriv_oi_signal']     = float(_OI_SIGNAL_MAP.get(oi_sig, 0.0))
    result['deriv_ls_ratio']      = float(np.clip((oi.get('ls_ratio', 1.0) - 1.0) / 2, -1, 1))
    ...
    # ── liquidation (3) ────────────────────────────────────────────────────
    result['liq_long_dist_pct']   = float(np.clip(liq.get('nearest_long_liq_dist_pct', 10.0) / 10, 0, 1))
    result['liq_short_dist_pct']  = float(np.clip(liq.get('nearest_short_liq_dist_pct', 10.0) / 10, 0, 1))
    result['liq_cascade_risk']    = float(np.clip(liq.get('cascade_risk_score', 0) / 100, 0, 1))

    # ── regime/sentiment (5) ───────────────────────────────────────────────
    result['regime_vol_mult']      = float(np.clip(atr.get('vol_regime_mult', 1.0), 0.5, 1.5))
    result['regime_fg_current']    = float(np.clip(sent.get('fg_current', 50) / 100, 0, 1))
    result['regime_fg_momentum_7d'] = float(np.clip(sent.get('fg_momentum_7d', 0) / 50, -1, 1))
    cc = rsi.get('rsi_centerline_cross', 'none')
    result['regime_rsi_centerline'] = 1.0 if cc == 'crossed_up' else (-1.0 if cc == 'crossed_down' else 0.0)
    result['regime_adx_normalized'] = float(np.clip(atr.get('atr_14', 0.0) / (current + 1e-9) * 100 / 50, 0, 1))
    ...
    result['onchain_whale_strength']  = float(np.clip(sent.get('whale_strength', 0.0), 0, 1))
    result['onchain_exchange_flow']   = float(np.clip(sent.get('exchange_flow', 0.0), -1, 1))
