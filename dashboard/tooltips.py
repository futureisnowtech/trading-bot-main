...
TIPS = {
    ...
    # ── New section cards (v17.1) ─────────────────────────────────────────────
    "bot_status": (
        "Bot Status: Is the system alive and running normally right now? "
        "Checks three things: (1) heartbeat — did the bot write an 'I am alive' signal recently, "
        "(2) scan freshness — did the scanner check markets in the last few minutes, "
        "(3) error count — are there unexpected failures in the last hour. "
        "Good = all three are fine. Watch = minor issues. Problem = something needs attention."
    ),
    "account_snapshot": (
        "Account Snapshot: Your full trading account at a glance. "
        "Equity = current account value including open unrealized P&L. "
        "Today's P&L = profit or loss since midnight today. "
        "Deployed = how much of the account is in active trades right now (as %). "
        "Drawdown = how far below your recent peak balance you are."
    ),
    "trade_quality": (
        "Trade Quality: Is the strategy actually making money after all costs? "
        "Profit Factor (PF) is the single most important number: total wins ÷ total losses. "
        "PF ≥ 1.35 = confirmed edge and safe to scale. "
        "PF 1.0–1.35 = profitable but fragile. PF < 1.0 = losing strategy. "
        "Win Rate and EV/trade tell the full story alongside PF."
    ),
    "biggest_issue": (
        "Biggest Issue Right Now: The single most important thing to pay attention to. "
        "Priority order: bot offline > high error rate > execution failures > "
        "losing money > account in drawdown > weak trade quality > scanner problems. "
        "If this shows Good, everything is running normally."
    ),
    "risk_overview": (
        "Risk Overview: How exposed is the account right now? "
        "Deployed = % of account in active trades. "
        "Drawdown = how far below peak. "
        "The bot enforces automatic risk limits: max 90% deployed, "
        "kill switch at 50% of live baseline (75% of paper baseline). "
        "This card tells you whether those limits are being approached."
    ),
    "decision_quality": (
        "Decision Quality: Are we choosing the right trades? "
        "Shows the outcome breakdown of closed trades: how many hit the target vs "
        "how many were stopped out. Also shows which signals are improving "
        "(Bayesian learning identified them as reliable) and which are degrading. "
        "Good decision quality = most losses come from valid stop-outs, not bad entries."
    ),
    "execution_quality": (
        "Execution Quality: Are we entering and exiting at good prices? "
        "Entry timing score = how well-timed our entries were (10 = entered right before the move). "
        "Exit efficiency = what fraction of each move we actually captured. "
        "Fee trap rate = % of trades where fees ate more than half the profit. "
        "Slippage is not yet fully instrumented."
    ),
    "learning_status": (
        "Learning Status: Is the bot improving from past trades? "
        "The ML (machine learning) model needs at least 200 clean trade snapshots before it activates. "
        "Until then, the system uses rule-based signals only — this is normal and expected early on. "
        "Once active, the ML tower scores every trade 0-100 using 57 market features, "
        "and Bayesian learning adjusts signal weights based on what's been winning and losing."
    ),
    "scanner_summary": (
        "Scanner Summary: Where are trade ideas getting filtered out? "
        "The scanner checks 200+ symbols across Kraken Futures, Binance, and Hyperliquid. "
        "It filters by: volume, signal score, economics (fee viability), and execution policy. "
        "Most ideas are filtered — this is intentional. Only high-quality setups reach entry. "
        "Conversion rate = % of scanned symbols that actually became trades."
    ),
    "failure_modes": (
        "Failure Modes: What has been going wrong in the last 7 days? "
        "This tracks categories of recurring failures — broker issues, data gaps, "
        "risk blocks, ML errors. Each category shows how many times it occurred "
        "and the severity. Zero failures = healthy system."
    ),
}
