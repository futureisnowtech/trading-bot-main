...
def _hours_to_resolution(value: str) -> float:
    ...
    try:
        ...
        raise CovarianceDataUnavailable(f"unparseable_resolution_time:{value}") from exc
...
def check_and_shrink_candidate(
    ...
    for pos in open_positions:
        ...
        enriched = {
            ...
            "yes_leg_entry_price": float(
                pos.get("yes_leg_entry_price")
                or pos.get("entry_price")
                or pos.get("entry")
                or 0.50
            ),
            "held_side_entry_price": pos.get("held_side_entry_price"),
            "market_exposure_usd": pos.get("market_exposure_usd"),
    ...
    for c in contracts_all:
        ...
        if w_data:
            ...
            try:
                hours_to_res = _hours_to_resolution(
                    c.get("resolution_at") or c.get("last_trade_at", "")
                )
                ...
                raise CovarianceDataUnavailable(
                    f"pricing_unavailable:{ticker}:{exc}"
                ) from exc
        else:
            raise CovarianceDataUnavailable(f"weather_data_unavailable:{ticker}")
