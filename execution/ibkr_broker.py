    ...
    def buy_mes(
        ...
        if self.is_connected():
            try:
                ...
                print(
                    f"[IBKRBroker] BUY {num_contracts} MES @ ~{current_price:.2f} | SL={stop_price} TP={target_price}"
                )
            except Exception as e:
            ...
            print(
                f"[IBKRBroker] ⚠️ Not connected — paper-logging BUY {num_contracts} MES @ {current_price:.2f}"
            )
    ...
    def short_mes(
        ...
        if self.is_connected():
            try:
                ...
                print(
                    f"[IBKRBroker] SHORT {num_contracts} MES @ ~{current_price:.2f} | SL={stop_price} TP={target_price}"
                )
            except Exception as e:
            ...
            order_id = f"IBKR_OFFLINE_{uuid.uuid4().hex[:8]}"
            print(
                f"[IBKRBroker] ⚠️ Not connected — paper-logging SHORT {num_contracts} MES @ {current_price:.2f}"
            )
