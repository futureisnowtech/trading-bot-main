...
## Current Version: v17.4 (2026-04-22)
...
- **Spot day-trading exits (v17.3):** `spot_engine.check_spot_targets()` closes position when `current_price >= open_positions.target` (3R target = entry × (1 + stop_pct × SPOT_TARGET_R)). `spot_engine.check_spot_eod_close()` flattens all spot positions at/after `SPOT_EOD_CLOSE_TIME` ET (default 15:45) on weekdays to prevent overnight gap exposure. Both called from `v10_runner` exit monitor every 30s alongside existing `check_spot_stops` / `check_spot_trailing` / `check_spot_thesis_exits`. `open_spot()` now persists real `target_price` (was 0.0) to `open_positions.target`. Config: `SPOT_TARGET_R=3.0`, `SPOT_EOD_CLOSE_TIME="15:45"`. Fee math: 1.2% round-trip, 3R target → break-even WR ~35% (well below economics gate score floor of 74). Spot economics gate (`risk/spot_economics_gate.py`) requires composite ≥ 55 AND edge > 2× fees — minimum passing score ≈ 74. Both BTC and ETH enter independently when score clears this bar.
