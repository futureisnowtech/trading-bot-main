...
- v19.6.0 (Sovereign Conviction Upgrade): Implemented a tiered alpha engine to capture long-range Grand Slam trades. Extended the discovery resolution window from 3 to 7 days. Tier 1 trades (>92% prob, <1.0F Sigma) now authorized for 20% sizing and 168h windows. Tier 2 trades (>85% prob, <1.5F Sigma) authorized for 15% sizing and 120h windows. Standard trades remain at 10% sizing and 72h.
...
- v19.4.0 (Sovereign Balance): Hardened the 31-city expansion with dynamic, balance-aware risk controls and automated maintenance. Replaced fixed dollar hub caps with a 20% of equity dynamic gate. Implemented a strict "One Strike Per Side" exclusivity rule for all weather events to prevent redundant bracket risk. Wired a 6-hour automated forensic DB pruning task (7-day quote retention) to maintain system performance and HUD dashboard responsiveness.
...
- v19.1.6 (Sovereign Weather Expansion): Horizontally expanded the weather lane to 15+ US cities (Austin, Phoenix, Seattle, etc.). Introduced Precipitation (`KXRAIN...`) markets utilizing GFS ensemble probabilities. Enforced a ruthless 72-hour maximum resolution window (`MAX_DAYS_TO_RESOLUTION = 3.0`) for all Forecast markets. Optimized QuoteHarvester with a throttled ThreadPoolExecutor. Implemented 6-hour coordinate-based caching for Open-Meteo ensemble data.

## 2026-05-27
- v19.1.4 (High-Velocity Weather Sovereign): Overhauled the Kalshi weather lane into a high-aggression scalping engine. Enforced a strict 72-hour maximum resolution gate across discovery and strategy evaluation. Integrated a weather-specific strategy family using 31-member GFS ensembles for edge detection (8% floor). Deployed a Logistic Sigmoid sizing model with a 25% bankroll cap for high-conviction weather plays. Tightened the execution cadence to 2-minute cycles to capture rapid market shifts.
- v19.1.3 (Sovereign Truth HUD realignment): Aligned lane IDs between bot and dashboard (unified on `crypto`). Fixed $0 equity bug by summing all broker holdings regardless of manual/bot classification. Resolved `NameError` in `v10_runner.py` by moving variable initialization to the function entry point. Mounted host `/proc` and `/sys` to containers for real-time hardware vitals. Enforced strict `YYYY-MM-DD HH:MM:SS` timestamp formatting in `server.py`.
