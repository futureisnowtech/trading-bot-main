...
# ── pre-push: canonical local release audit ───────────────────────────────────
cat > "$HOOKS_DIR/pre-push" << 'HOOK'
#!/bin/bash
# pre-push hook: canonical local release gate before pushing
REPO_ROOT="$(git rev-parse --show-toplevel)"
echo ""
echo "Running pre-push gate..."

echo "  [1/1] Local release audit..."
python3 "$REPO_ROOT/scripts/release_audit.py" --local
if [ $? -ne 0 ]; then
    echo ""
    echo "❌ Local release audit failed. Fix blockers before pushing."
    exit 1
fi
...
echo "  ✅ pre-push hook installed (release_audit.py --local)"
