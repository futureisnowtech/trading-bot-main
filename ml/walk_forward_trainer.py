...
def _load_training_data(
    pair_key: str, direction: str, paper: bool = True
...
def _compute_metrics(y_true: np.ndarray, y_pred: np.ndarray, pnls: np.ndarray) -> Dict:
    """
    Compute WR, PF, Sharpe, max DD from regressor predictions.

    y_pred is predicted_pnl (dollar amount, not probability).
    We simulate taking a trade whenever predicted_pnl > 0.
    y_true is actual pnl_usd (used only to compute WR).
    """
    # Take trade when model predicts positive PnL
    traded_mask = y_pred > 0.0

    if traded_mask.sum() < 5:
        return {"wr": 0, "pf": 0, "sharpe": 0, "max_dd": 1.0, "n_trades": 0}

    traded_pnls = pnls[traded_mask]
    traded_won = (y_true[traded_mask] > 0).astype(float)
    ...
    if len(traded_pnls) > 1:
        sharpe = float(
            np.mean(traded_pnls) / (np.std(traded_pnls) + 1e-9) * np.sqrt(252)
        )
    else:
        sharpe = 0.0
...
def _fold_passes(metrics: Dict) -> bool:
    return (
        metrics["wr"] >= _MIN_WR
        and metrics["pf"] >= _MIN_PF
        and metrics["sharpe"] >= _MIN_SHARPE
        and metrics["max_dd"] <= _MAX_DD
        and metrics["n_trades"] >= _MIN_TRADES
    )


def _train_xgb(X_train: np.ndarray, y_train: np.ndarray, params: Optional[Dict] = None):
    """Train XGBoost PnL regressor (target = net_pnl in USD)."""
    if not _XGB_OK:
        return None
...
def _train_lgbm(
    X_train: np.ndarray, y_train: np.ndarray, params: Optional[Dict] = None
):
    """Train LightGBM PnL regressor (target = net_pnl in USD)."""
    if not _LGB_OK:
        return None
...
def _optuna_optimize(
    X: np.ndarray, y: np.ndarray, model_type: str = "xgb", n_trials: int = 20
) -> Optional[Dict]:
    """
    ...
    def objective(trial):
        ...
        for train_idx, val_idx in tscv.split(X):
            ...
            try:
                ...
                if m is None:
                    continue
                # Regressor: y = pnl_usd; take trade when predicted_pnl > 0
                preds = m.predict(X[val_idx])
                pnl_proxy = np.where(preds > 0, y[val_idx], 0.0)
...
def train_walk_forward(
    ...
    # Target: continuous PnL (dollars). Regressor learns to predict net outcome.
    # pnl_scale = 1 std of training PnL → used by model_store for tanh normalization.
    pnls = df["pnl_usd"].values
    y = pnls  # regression target
    ...
    while True:
        ...
        # Regressors: predict dollar PnL directly
        if xgb_model is not None and lgb_model is not None:
            ensemble_pred = 0.6 * xgb_model.predict(X_vl) + 0.4 * lgb_model.predict(
                X_vl
            )
        elif xgb_model is not None:
            ensemble_pred = xgb_model.predict(X_vl)
        elif lgb_model is not None:
            ensemble_pred = lgb_model.predict(X_vl)
        else:
            current_train_start += pd.Timedelta(days=step)
            continue

        metrics = _compute_metrics(y_vl, ensemble_pred, pnl_vl)
    ...
    if pass_rate < _PASS_PCT:
        result["reason"] = (
            f"walk-forward failed: {n_passed}/{n_folds} folds passed "
    ...
    # Save pnl_scale metadata — used by model_store for tanh normalization
    if saved:
        meta_path = _model_path(pair_key, direction, "meta")
        with open(meta_path, "wb") as f:
            pickle.dump({"pnl_scale": pnl_scale}, f)
