...
def get_bot_activity(n=40):
    ...
    for line in reversed(lines):
        ...
        if not any(
            x in line
            for x in ("[v10]", "[scanner]", "[perps]", "[risk]", "[wft]", "[learning]")
        ):
            continue
        if any(
            x in line
            for x in ("ib_insync", "IBKRBroker", "Connecting to", "Disconnecting")
        ):
            continue
...
def render_status():
    ...
    deployed = sum(float(p.get("qty", 0)) * float(p.get("entry", 0)) for p in open_p)
    ...
    c3.metric(
        ...
        delta=f"${realized_pnl:+.2f} realized · ${unrealized_pnl:+.2f} open"
        if abs(unrealized_pnl) > 0.01
        else f"{_fmt_pnl(realized_pnl)} realized",
    ...
    st.caption(
        f"Refreshed {datetime.now().strftime('%H:%M:%S')} · Paper session started {LAUNCH_DATE} · Backtest / contaminated data excluded"
    )
...
def render_portfolio():
    ...
    c3.metric("Profit Factor", pf_str, delta="need ≥1.35 for live")
    c4.metric("Total Fees", _fmt_pnl(-stats["total_fees"]))
    c5.metric("Clean Trades", f"{stats['closes']} / 30", delta="minimum for ML gate")
    ...
    if eq:
        ...
        st.line_chart(
            df.set_index("ts")[["Cumulative P&L ($)"]],
            height=200,
...
def render_open_positions():
    ...
    def _pnl_cell(v):
        ...
        return (
            ...
            if v > 0
            else ("color: #f87171; font-weight:600" if v < 0 else "color: inherit")
        )
...
def render_scanner():
    ...
    if _run_now:
        ...
        try:
            ...
            try:
                ...
                _acct_sz = 10000
            _live_candidates = _sc.scan(
                open_positions=_open_syms, account_balance=float(_acct_sz)
            )
            if _live_candidates:
                _scan_status_placeholder.success(
                    f"Scan complete — {len(_live_candidates)} candidates found"
                )
                ...
                _display_cols = [
                    ...
                    if c in _cand_df.columns
                ]
                st.dataframe(
                    _cand_df[_display_cols] if _display_cols else _cand_df,
                    use_container_width=True,
                    hide_index=True,
                )
            else:
                _scan_status_placeholder.warning(
                    "Scan complete — 0 candidates passed all filters"
                )
    ...
    if scan["candidates"]:
        ...
        st.dataframe(
            pd.DataFrame(scan["candidates"]), use_container_width=True, hide_index=True
        )
    else:
    ...
    with st.expander("Scanner config (live values from scanner.py)"):
        try:
            ...
            cfg = {
                ...
                "Source": "Kraken Futures + Binance USDM + Hyperliquid (3 sources, every scan cycle)",
            }
...
def render_signal_brain():
    ...
    # ── Entry thresholds ──────────────────────────────────────────────────────
    st.caption(
        "**Tier 2 entry thresholds by regime** (composite score must meet these; Tier 1 bypasses)"
    )
    try:
        ...
        thresh_rows.append(
            {"Regime": "Tier 1 size mult", "Min Score": "1.0× base (full)"}
        )
        st.dataframe(
            pd.DataFrame(thresh_rows), use_container_width=False, hide_index=True
        )
    except Exception as e:
    ...
    st.caption(
        f"**Bayesian signal learning** — {len(bay_stats)} signals tracked · updates after every closed trade · PRIOR_N=20 · live after 10 fires"
    )
    if bay_stats:
    ...
    with st.expander("Technical tower — all scoring conditions (LONG side)"):
        ...
        st.caption(
            "Raw range ~−115 to +150 · normalized 0–100 · same set mirrored for SHORT side"
        )
        import pandas as pd
    ...
    with st.expander("CHOP index gating"):
        ...
        st.text(
            "CHOP > 61.8  →  RANGING   →  momentum setups blocked; ranging_mr_long/short active"
        )
    ...
    status = (
        "ACTIVE — models being trained"
        if snaps >= needed
        else f"ACCUMULATING — {snaps}/{needed} snapshots"
    )
    st.metric("ML Tower — 57-feature snapshots", f"{snaps} / {needed}", delta=status)
    st.caption(
        "XGBoost 60% + LightGBM 40% · walk-forward 60d train / 10d val · WR≥54%, PF≥1.35, Sharpe≥0.8 to pass"
    )
...
def render_system_config():
    ...
    with col_left:
        ...
        try:
            ...
            st.text(
                f"  Ranging R:R floor:       ≥ {_MIN_NET_RR * 1.25:.2f}:1 (CHOP > 61.8, 1.25× tighter)"
            )
            st.text(
            ...
            st.text(
                f"  Ranging EV floor:        ≥ {_TIER_B_EV * 1.67 * 100:.3f}% (1.67× tighter)"
            )
        except Exception as e:
        ...
        try:
            ...
            st.text(
                f"  Portfolio heat cap:      {MAX_HEAT_PCT * 100:.0f}%  = ${acct * MAX_HEAT_PCT:.0f} max deployed"
            )
            st.text(
    ...
    with col_right:
        # Exit stack
        st.caption(
            "**6-priority exit stack** — position_manager.py (highest number = highest priority)"
        )
        exits = [
            ...
            (
                "5",
                "Risk Forced Exit",
                "Margin breach / portfolio VaR breach / correlation limit",
            ),
            ("4", "Hard Stop", "STOP_MARKET at entry − ATR×1.5 · NEVER widened"),
            (
                "3",
                "Thesis Invalidated",
                "composite < entry_score × regime_pct → close · 10 min hold gate (TRENDING=30%, RANGING=15%, HIGH_VOL=35%, default=25%)",
            ),
            (
                "2",
                "TP Scale-Out",
                "2R → close 33% · 3.5R → close 33% · remainder trails",
            ),
            (
                "1",
                "Trailing Stop",
        ...
        ml = get_ml_status()
        st.text(
            f"  57-feature snapshots stored:  {ml['snapshots']} / {ml['min_needed']} needed"
        )
...
def _win_prob(c: dict) -> float:
    ...
    if fund > 0.002:
        prob += 3  # >0.2% annualized, elevated
    elif fund > 0.0005:
        prob += 1
...
def _win_prob_breakdown(c: dict) -> list:
    ...
    # Volume
    if vs >= 3.0:
        rows.append(("Volume spike", f"{vs:.2f}× avg  (strong conviction)", "+9%"))
    elif vs >= 2.0:
        rows.append(("Volume spike", f"{vs:.2f}× avg  (moderate conviction)", "+6%"))
    elif vs >= 1.5:
        rows.append(("Volume spike", f"{vs:.2f}× avg  (mild conviction)", "+3%"))
    else:
        rows.append(("Volume spike", f"{vs:.2f}× avg  (weak — minimal edge)", "+0%"))
    ...
    if "momentum" in setup and adx >= 25:
        rows.append(
            (
                "ADX trend strength",
                f"{adx:.1f}  (strong trend confirms momentum entry)",
                "+7%",
            )
        )
    elif "ranging" in setup and adx < 20:
        rows.append(
            (
                "ADX trend strength",
                f"{adx:.1f}  (low ADX confirms ranging/mean-rev setup)",
                "+7%",
            )
        )
    elif "kst" in setup and adx < 30:
        rows.append(
            (
                "ADX trend strength",
                f"{adx:.1f}  (moderate trend, KST cross valid)",
                "+4%",
            )
        )
    else:
    ...
    if st_ok:
        rows.append(
            ("SuperTrend direction", f"{st_label}  ✓ aligned with {dirn}", "+5%")
        )
    else:
    ...
    if "ranging" in setup:
        if vwap_d >= 2.0:
            rows.append(
                (
                    "VWAP displacement",
                    f"{vwap_d:.2f}%  (strongly extended → snap-back likely)",
                    "+5%",
                )
            )
        elif vwap_d >= 1.0:
            rows.append(
                ("VWAP displacement", f"{vwap_d:.2f}%  (moderately extended)", "+3%")
            )
        else:
            rows.append(
                (
                    "VWAP displacement",
                    f"{vwap_d:.2f}%  (close to VWAP — weak MR edge)",
                    "+0%",
                )
            )
    ...
    if fund > 0.002:
        rows.append(
            (
                "Funding rate",
                f"{fund * 100:.4f}% ann  (elevated — squeeze pressure adds edge)",
                "+3%",
            )
        )
    elif fund > 0.0005:
    ...
    if dirn == "LONG" and pm1h > 0.3:
        ...
        rows.append(
            ("1h price momentum", f"{pm1h:.2f}%  (neutral or against direction)", "+0%")
        )
...
def _render_trade_details(c: dict, prob: float):
    ...
    st.text(
        f"  Funding cost (est): {fund_cost:.4f}%  ({abs(fund_ann) * 100:.4f}% ann ÷ 365×3 × hold)"
    )
    st.text(f"  Net win if TP hit:  {net_win * 100:.3f}%  →  ${net_win * pos_usd:+.2f}")
    st.text(
        f"  Net loss if SL hit: {net_loss * 100:.3f}%  →  ${-net_loss * pos_usd:.2f}"
    )
    st.text(
        f"  EV = 52%×${net_win * pos_usd:.2f} − 48%×${net_loss * pos_usd:.2f}  =  ${ev:+.2f}"
    )
    ...
    c2.text(
        f"  SuperTrend dir:  {'↑ bullish' if c.get('supertrend_dir', 0) > 0 else ('↓ bearish' if c.get('supertrend_dir', 0) < 0 else 'flat')}"
    )
            ...
            try:
                ...
                if live_now > 0:
                    ...
                    if 0.90 <= ratio <= 1.10:
                        ...
                        st.warning(
                            f"⚠️ {sym}: candle price ${candle_price:.5g} was "
...
def render_futures():
    ...
    with col_l:
        ...
        if or_locked and or_high and or_low:
            ...
            st.caption(
                f"Long breakout trigger: ≥ {long_entry}  |  Short breakdown trigger: ≤ {short_entry}"
            )
...
def main():
    ...
    st.caption(
        "Two-tower signal engine · 6-priority exit stack · 57-feature ML · Kraken + Hyperliquid perps · MES futures"
    )
