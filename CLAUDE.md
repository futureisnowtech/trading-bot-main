...
- **Balance total_usd_balance (v16.4):** Coinbase CFM API returns `total_usd_balance.value = "0"` (string, truthy) when account has no total balance field. Old code: `float("0" or equity) = 0.0`. Fix: convert first, then fall back to `futures_buying_power` if result ≤ 0. Dashboard now shows real buying power (~$1,966) instead of $0.
