...
| Spot execution | `spot_engine.py` → `execution/coinbase_spot_broker.py` | Coinbase spot scalp lane with 8-symbol broker support and setup-first long-only symbol policies; risk-from-stop sizing; maker-first/taker-fallback route; restart-safe spot state persisted |
| Spot strategy policy | `runtime/spot_strategy.py` | Replay-derived per-coin edge policy for the 8-symbol spot lane: exact edge conditions, exit profiles, shared quality gating, and shared policy summaries for autonomous + manual + dashboard paths |
...
- **Spot scalp model (v17.4):** spot lane now supports `BTC`, `ETH`, `SOL`, `XRP`, `LTC`, `DOGE`, `ADA`, and `LINK` long-only. Crypto spot is 24/7 state-gated, not clock-gated. Entries blend the existing composite with a multi-timeframe derivative score (`5m/30m/4h/1d`) built from KST + Ichimoku + SuperTrend + MACD/VWAP/returns. Regime (`TREND`/`NEUTRAL`/`CHOP`) controls entry floor, stop width, target R, trail arm, and cooldown.
- **Spot sizing / exits (v17.4):** live spot sizing path in `scheduler/v10_runner.py` is now risk-from-stop based instead of `6%–10% of balance`. Stops are ATR/instability-based with symbol clamp bands, targets are regime-based (`1.8R/1.2R/0.9R`), and spot exits now include hard stop, trailing stop, target hit, stagnation exit, and thesis decay on a dedicated 5-second spot loop.
- **Spot execution truth (v17.4):** `execution/coinbase_spot_broker.py` now supports 8-symbol product specs, top-of-book reads, limit post-only spot orders, order-status polling, cancel, fill aggregation, and best-effort fee truth. `spot_engine.py` persists execution route (`maker_first` / `taker_fallback` / `skipped_microstructure`) and stop/target model versions into `open_positions`.
...
- **Setup-first spot policy (v18.0):** the spot lane is no longer a rigid “primary/secondary setup only” system. `runtime/spot_strategy.py` now defines a finite setup library (`impulse_continuation`, `pullback_reclaim`, `compression_breakout`, `trend_resume_after_shakeout`, `compression_expansion_retest`), symbol-specific preferred setups, and dynamic opportunistic/wildcard allowances based on `setup_score`. Live spot quality gating now uses setup policy first, then score/risk/economics, instead of treating one blended score as the whole strategy.
- **Persisted setup evidence (v18.0):** `scan_candidates` and `open_positions` now persist `setup_score` and `setup_preference` alongside `setup_family`, `final_spot_score`, and `regime_floor`. The CRYPTO dashboard and manual spot controls render those stored metrics directly so operators can see how strong the live setup evidence was without re-deriving it in Streamlit.
- **Setup-aware quality/economics alignment (v18.0):** `runtime/spot_strategy.py`, `runtime/spot_regime.py`, and `risk/spot_economics_gate.py` now share the same setup-aware floor semantics. Preferred, opportunistic, and wildcard spot setups use the same regime-floor logic in autonomous quality gating, economics vetoing, manual preview, and dashboard truth.
- **Research strategy extractor (v18.1):** `backtesting/spot_replay_optimizer.py` now includes an expectancy-first strategy extraction pass that scores replay trials by utility instead of raw win rate, interpolates family/local policy centroids, and emits a per-symbol strategy research artifact. Zero-trade and too-thin candidates are now penalized and filtered from the main frontier so “inactive” policies do not outrank tradable-but-imperfect ones during spot strategy research.
- **Per-coin strategy surgery (v18.2):** `backtesting/spot_replay_optimizer.py` now includes a per-symbol coordinate-search optimizer that starts from each coin’s own replay surface, explores local threshold/setup/regime/exit tweaks, and emits baseline-vs-optimized scorecards plus parameter deltas. The surgery pass is research-only, compares all candidate improvements against a shared symbol baseline, and caps tiny-sample profit-factor blowups so one-trade mirages do not dominate the optimization surface.
- **Replay-derived edge policy translation (v18.3):** the live spot lane now reads exact per-coin edge conditions from replay research instead of leaning on the old preferred/opportunistic setup abstraction alone. `config.py` now stores each coin’s edge profile (`quick`, `balanced`, `micro`) plus concrete edge filters such as `setup_family`, `setup_score`, `structure`, `vol_quality`, `a5`, and `mom_impulse`, and `runtime/spot_strategy.py` evaluates those conditions directly for autonomous quality gating.
- **Shared replay edge truth in operator surfaces (v18.3):** the same replay-derived edge policy is now surfaced in the manual spot panel and CRYPTO candidate cards. Operators can see the active edge profile, the exact edge filter summary, and the replay reference metrics (`PF`, `WR`, `n`) for each coin without the dashboard fabricating decisions client-side.
- **Current-strategy dashboard epoch (v18.4):** dashboard “current” crypto metrics now clamp to a canonical strategy epoch via `dashboard/db.py:get_current_strategy_start_date()` instead of mixing pre-rollout history into current-state KPIs. `dashboard/data/execution.py`, `dashboard/data/trading_control.py`, and `dashboard/data/crypto_dashboard.py` all use that epoch-aware cutoff so fee-trap rate, funnel stats, and crypto overview metrics default to post-rollout truth while preserving older rows for research/history views.
- **Combined spot+perp deployed truth (v18.4):** `dashboard/data/positions.py:get_crypto_deployed_snapshot()` is now the canonical deployed-capital reader for crypto. `dashboard/data/control_tower.py`, `dashboard/data/scanner_data.py`, and `scheduler/v10_runner.py` use it or mirror its semantics so overview/runtime/control surfaces stop showing `$0` deployed when spot positions are open and instead persist/read combined live spot + perp deployment and open-count truth.
- **Simplified Control Tower (v18.5):** the main dashboard page is now intentionally trimmed to current operator truth instead of acting like a diagnostics warehouse. `dashboard/widgets/pages/control_tower.py` now defaults to a compact current-rollout board: bot status, account snapshot, current-strategy stats, biggest issue, open positions, current pipeline, activity, and recent closed trades. Older clutter such as lane-role cards, performance curve, decision/execution/learning blocks, funnel-detail bars, alerts, and launch-era trade summaries were removed from the main surface.
...
- **Advanced spot state math (v17.7):** `runtime/spot_momentum.py` now computes a higher-order spot state from the repo-native indicator stack. In addition to `z/v/a`, each timeframe now tracks jerk, momentum/acceleration path integrals, path efficiency, participation quality, volatility quality, and richer directional structure from WAE, squeeze state, ADX, dollar-volume pressure, KST, Ichimoku, SuperTrend, MACD, AVWAP, and returns. Live spot scoring still writes the same truth fields, but the derivative engine is now materially more expressive.
- **Symbol-specific spot strategies (v17.9):** the spot lane is no longer one global scalp template with a couple of overrides. `runtime/spot_strategy.py` now gives each of `BTC`, `ETH`, `SOL`, `XRP`, `LTC`, `DOGE`, `ADA`, and `LINK` its own live policy surface: allowed regimes, allowed setup families, regime floors, blend weights, minimum confirmation count, minimum frame scores, minimum path efficiency, participation/volatility quality thresholds, and regime-specific target/trail maps.
- **8-symbol autonomous spot routing (v17.9):** `config.SPOT_STRATEGY_SYMBOLS` is back to the full 8-name spot universe, but autonomy is now controlled by each symbol’s strategy policy rather than one shared lane shape. `scheduler/v10_runner.py`, `spot_engine.py`, and `runtime/crypto_tradeability.py` all consult the same symbol strategy layer before scoring or routing a spot trade.
- **Replay-fitted spot philosophy (v17.8 → v17.9):** the old BTC/LINK narrowing became the seed for a broader architecture change instead of the final answer. The live system now supports per-symbol spot strategy fitting, while the replay engine remains the source of truth for whether a symbol’s policy is actually good enough to trust.
...
- **Crypto header/opportunity-board truth (v17.4):** CRYPTO header open-count and deployment now come from the live perp + spot readers instead of raw `open_positions` counts and entry notional. `scan_candidates` journaling/dashboard queries now use canonical fields (`base_asset`, `scanner_expected_profit`) and persist/read structured spot scalp truth columns.
- **Dashboard live perp truth (v17.3 hotfix):** `dashboard/data/positions.py:get_perp_positions()` now treats Coinbase CFM `/api/v3/brokerage/cfm/positions` as canonical in live mode and only borrows DB metadata (stop/target/ts_entry/notes) for matching symbols. Closed perps that linger in `open_positions` no longer appear as active dashboard trades when the live futures account is flat.
- **Dashboard live spot truth (v17.3 hotfix):** `dashboard/data/positions.py:get_spot_positions_dashboard()` now treats Coinbase spot holdings from `execution/coinbase_spot_broker.py` as canonical in live mode and only enriches them with DB metadata for matching symbols. Live spot rows no longer depend on stale `open_positions` quantities.
- **Dashboard live stocks truth (v17.3 hotfix):** `dashboard/data/stocks.py` now uses a dedicated read-only IBKR dashboard client (`IBKR_STOCK_DASHBOARD_CLIENT_ID`, default `14`) to fetch live stock positions/account value. If that snapshot cannot be obtained, the STOCKS page fail-closes to `connected=false`, `open_count=0`, `account_value=0.0` instead of showing stale DB positions.
...
- **Spot replay optimizer (v17.7 → v17.9):** `backtesting/spot_replay_optimizer.py` is the research-grade spot optimizer for the 8-symbol scalp lane. It backfills deep `5m` spot history through reachable public venues, resamples that same path into `30m/4h/1d`, rebuilds the live derivative/regime/setup state at each evaluation point, simulates stop/target/trailing outcomes on future `5m` bars, and now supports symbol-specific fitting via curated per-symbol trial banks. It can emit recommended live policies per symbol (`allowed_regimes`, `allowed_setups`, floors, weights, quality thresholds, target/trail maps) while keeping outputs strictly research-only under `logs/backtest/`.
- **Dynamic setup replay fitting (v18.0):** `backtesting/spot_replay_optimizer.py` now fits broader spot-policy surfaces using dynamic setup modes (`preferred_only`, `preferred_dynamic`, `reclaim_dynamic`, `breakout_dynamic`, `momentum_dynamic`) instead of only rigid allowed-setup lists. Replay events now carry `setup_family` + `setup_score`, and trial evaluation can reward symbols for preferred setups while still allowing exceptional opportunistic setups when derivative evidence is strong enough.
...
| v18.4 | 2026-04-25 | Dashboard current-state truth pass: current crypto/control/execution metrics now clamp to the strategy epoch, combined spot+perp deployed capital is canonical across overview/runtime/scanner surfaces, and historical rows remain preserved for research views |
