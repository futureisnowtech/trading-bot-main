...
def _get_db() -> sqlite3.Connection:
    ...
    """)
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_ohlcv_lookup ON ohlcv(symbol, timeframe, open_time)"
    )
    conn.commit()
    return conn
...
def _fetch_binance(
    symbol: str, interval: str, start_ms: int, limit: int
) -> Optional[list]:
...
def _fetch_kraken(symbol: str, interval: str, limit: int) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        df["open_time"] = pd.to_datetime(
            df["open_time"].astype(int), unit="ms", utc=True
        )
        df = df.set_index("open_time")
        return df.tail(limit)
    except Exception as e:
        logger.debug(
            f"[historical_data] Kraken fetch failed for {symbol} {interval}: {e}"
        )
        return None
...
def _load_from_db(
    ...
    with _lock:
        try:
            ...
            logger.debug(
                f"[historical_data] cache unavailable ({e}), falling through to live fetch"
            )
            return pd.DataFrame()
        try:
...
def get_candles(symbol: str, timeframe: str = "5m", limit: int = 200) -> pd.DataFrame:
    ...
    if not is_binance_fmt:
        ...
        if hl_df is not None and not hl_df.empty:
            logger.debug(
                f"[historical_data] HL OK {symbol} {timeframe} ({len(hl_df)} bars)"
            )
            return hl_df
        ...
        if base and base != symbol:
            ...
            if hl_df is not None and not hl_df.empty:
                logger.debug(
                    f"[historical_data] HL fallback OK {symbol}→{base} {timeframe}"
                )
                return hl_df
    ...
    if not cached.empty:
        logger.debug(
            f"[historical_data] Partial cache ({len(cached)}/{limit}) for {symbol}"
        )
        return cached.tail(limit)
...
def _fetch_and_store(
    symbol: str, timeframe: str, start_ms: int, limit: int, bar_ms: int, now_ms: int
...
def prefetch_symbols(symbols: list, timeframes: list = None):
    ...
    logger.info(
        f"[historical_data] Prefetching {len(symbols)} symbols × {len(timeframes)} timeframes"
    )
    for sym in symbols:
