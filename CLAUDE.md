...
## Current Version: v18.13 (2026-04-29)
...
| Spot execution | `spot_engine.py` → `execution/coinbase_spot_broker.py` | Coinbase spot 8-symbol scalp lane (BTC/ETH/SOL/XRP/LTC/DOGE/ADA/LINK); no leverage, no shorting; gated by SPOT_LANE_ACTIVE; 50% deployment cap; maker-first routing |
| Spot state engine | `runtime/spot_momentum.py` | Multi-timeframe spot state (5m/30m/4h/1d): derivative stack z/v/a, KST+Ichimoku+SuperTrend confirmations, blended composite+derivative score |
| Spot regime | `runtime/spot_regime.py` | Classifies regime TREND/NEUTRAL/CHOP from 30m+4h frame scores |
| Spot execution policy | `runtime/spot_execution_policy.py` | Maker-first / taker-fallback price helpers; maker poll count |
| Stocks execution | `scheduler/stock_runner.py` → `execution/ibkr_stock_broker.py` | IBKR US equity swing trades; clientId=4; port 7496 (live); gated by STOCKS_LANE_ACTIVE; 8-signal technical tower; bracket orders (entry limit + server-side stop + GTC target) |
...
| Trading control plane | `dashboard/data/trading_control.py` + `dashboard/widgets/system_settings/master_control.py` | Master SYSTEM SETTINGS panel showing crypto funnel, blocker categories, forecast contradictions, and truth checks |
...
| Dashboard | `dashboard/app.py` | Streamlit Operator Panel, 5 tabs: CONTROL TOWER (default), CRYPTO, FORECAST, PERFORMANCE LAB, ENGINEERING CONSOLE |
| Dashboard pages | `dashboard/widgets/pages/` | Page-level composition layer: control_tower.py, crypto_page.py, forecast_page.py, performance_lab.py, engineering_console.py |
| Dashboard data orchestrators | `dashboard/data/control_tower.py`, `dashboard/data/crypto_dashboard.py`, `dashboard/data/engineering_console.py` | Compose existing readers into page-level snapshots; prefer persisted/runtime truth |
...
- **Forecast truth-layer primary source (v15.9 → v16.16):** `dashboard/data/forecast.py:get_forecast_health()` uses `lane_runtime_state` as primary truth, but `lane_started` is only true when `active=1` AND the heartbeat is fresh (180s threshold). `scripts/validate.py` now reports `ACTION NEEDED` when runtime says forecast is active but the heartbeat is stale.
...
- **Manual scan fail-closed (v16.2 → v16.16):** `dashboard/widgets/trade_approval/manual_scan.py` still scans `core_only=True`, but candidate rows, preview, final execute, and direct spot controls now all defer to shared tradeability. If the tradeability engine fails, the dashboard blocks execution with `execution_policy_unavailable`.
...
- **Manual scan truth closure (v16.16):** `manual_scan.py` no longer relies on row-local tier heuristics for executability. Row labels, preview, final execute, and direct spot controls all call the shared tradeability engine. `render_spot_section()` is always visible in trade approval, and direct spot buy/sell feedback persists through reruns using `st.session_state["spot_manual_message"]`.
- **Paper spot balance truth (v16.16):** `dashboard/data/balance.py:get_spot_balance_summary()` uses `_paper_spot_balance_summary()` in paper mode so the dashboard reflects DB-backed spot holdings and remaining USD instead of zero placeholders.
- **Dashboard DB alias unification (v16.16):** `dashboard/db.py` now aliases both `db` and `dashboard.db` to the same module object with `sys.modules.setdefault(...)`, preventing mixed imports from reading different `DB_PATH` values inside one process.
- **Master trading control (v16.16):** `dashboard/data/trading_control.py` + `dashboard/widgets/system_settings/master_control.py` provide one SYSTEM SETTINGS control plane that answers where trades are dying and whether the root cause is strategy, system/policy, or bug/integrity. Uses `scan_funnels`, `scan_candidates`, `lane_runtime_state`, and forecast data readers.
- **Forecast runtime truth (v16.16):** `forecast/runner.py` writes richer `lane_runtime_state` fields during startup/discovery (`connected`, `tradable`, `blocked_reason`, `readiness_state`, `action_needed`) so dashboard and validator surfaces read runtime truth instead of guessing from partial evidence.
- **Crypto tradeability backfill (v16.16):** `logging_db/trade_logger.py:init_db()` backfills blank `scan_candidates` tradeability fields with `tradeability_status='not_evaluated'` and preserved lane hints, eliminating blank recent truth rows in the control plane.
- **Dashboard architecture (v17.0):** 5-tab control-tower layout. CONTROL TOWER = default operator surface (global status strip + summary cards + funnel + live opportunities + open positions + lane health + action center). CRYPTO = unified spot/perp/manual-trade workflow (opportunity board + manual trade console + open positions + failure diagnostics + scanner detail). FORECAST = dedicated page with stale-heartbeat truth banner. PERFORMANCE LAB = strategy learning/attribution analysis. ENGINEERING CONSOLE = raw config/thresholds/integrity views + archived MES. Page composition layer: `dashboard/widgets/pages/`. Data orchestrators: `dashboard/data/control_tower.py`, `dashboard/data/crypto_dashboard.py`, `dashboard/data/engineering_console.py`. Dashboard truth rules: (1) persisted DB truth first, (2) runtime state second, (3) derived summaries labeled as such.
- **Live account-size truth (v17.1):** `runtime/live_account.py:get_live_account_size()` is the single denominator helper for paper vs live. Live mode reads `system_runtime_state.account_size_live` once the runner has persisted the real Coinbase balance; paper mode still uses `config.ACCOUNT_SIZE`. Live code must not read `config.ACCOUNT_SIZE` directly for deployment / kill-threshold math.
- **Crypto lane runtime-state closure (v17.1):** `scheduler/v10_runner.py` now writes real crypto lane fields (`connected`, `tradable`, `positions_open`, `capital_deployed_usd`, `buying_power_usd`, `readiness_state`) into `lane_runtime_state`. `main.py` seeds crypto as `STARTING`, so dashboard/launcher surfaces do not mistake startup placeholders for live truth.
- **Live risk-floor alignment (v17.1):** `position_manager.check_exits()` no longer hardcodes `75% * ACCOUNT_SIZE` for live risk-forced exits. Live exits now reference the live-baseline policy from `kill_switch.py`; paper mode retains the 75% paper floor.
- **Historical-data BTC truth (v17.1):** `data/historical_data.py` no longer misclassifies bare `BTC`/`ETH`-style symbols as Binance-format pairs. Bare core symbols route through a crypto-safe alias before any raw-symbol yfinance fallback, preventing stock/ETF collisions from screening out live BTC trades.
- **Controlled live launch verification (v17.1):** `scripts/go_live.py` no longer treats `process_mode=live` as sufficient. It now waits for crypto lane runtime truth to show `connected=1` and `buying_power_usd>0` before declaring the transition successful.
- **Stocks lane (v17.2):** `execution/ibkr_stock_broker.py` (clientId=4, port 7496) + `scheduler/stock_runner.py`. Standalone — zero imports from signal_engine/scanner/v10_runner. 8-signal technical tower: EMA alignment (+20), SuperTrend (+15), MACD hist rising (+15), RSI 45–70 (+15), volume surge (+10), above VWAP (+10), CHOP<50 (+10), above SMA20 (+5). Entry threshold: 60/100. Position sizing: 2% risk (STOCKS_RISK_PCT), 1.5×ATR stop, 3R target, 15% max per position (STOCKS_MAX_POSITION_PCT), 3 max concurrent (STOCKS_MAX_POSITIONS). Bracket orders: entry LimitOrder DAY + target LimitOrder GTC + stop StopOrder GTC, outsideRth=False. Paper mock fills when PAPER_TRADING=True. run_every=30m during market hours 9:30–16:00 ET weekdays. STOCKS_LANE_ACTIVE gates the lane (default false). Dashboard: STOCKS tab in app.py between CRYPTO and FORECAST.
- **Spot day-trading exits (v17.3):** `spot_engine.check_spot_targets()` closes position when `current_price >= open_positions.target`. `spot_engine.check_spot_eod_close()` flattens all spot positions at/after `SPOT_EOD_CLOSE_TIME` ET (default 15:45) on weekdays. Both called from `v10_runner` exit monitor every 30s. `open_spot()` now persists real `target_price` (was 0.0) to `open_positions.target`. Config: `SPOT_TARGET_R=1.2` default (env-overridable), runtime uses `SPOT_TARGET_R_BY_REGIME` table (TREND=0.85, NEUTRAL=0.65, CHOP=0.50), `SPOT_EOD_CLOSE_TIME="15:45"`. Note: v17.3 note originally said SPOT_TARGET_R=3.0 — that was superseded by v17.4 regime-table logic.
- **Self-calibrating spot edge conditions (v17.5):** Hardcoded `edge_conditions` removed from all 8 symbols in `SPOT_SYMBOL_STRATEGY_OVERRIDES` in `config.py` — they were derived from n=12–47 backtest trades and were blocking 100% of live entries. Replaced with `learning/spot_edge_calibrator.py` which derives conditions from real closed spot trades (requires `MIN_TRADES_PER_SYMBOL=30` per symbol). Conditions written to `spot_edge_conditions` DB table (new in `trade_logger.py`). `runtime/spot_strategy.py:_load_db_conditions()` reads DB-derived conditions at runtime; returns `None` (open gate) if calibrator has never run for a symbol, overrides config `edge_conditions` if it has. `monitoring/nightly_audit.py` now calls `_check_spot_edge_calibration()` to run calibration nightly. Until 30 real trades accumulate per symbol, all 8 spot symbols have open gate (empty `edge_conditions` tuple). 422 proof tests pass.
- **Spot lane governance (v18.11):** Evidence-derived quarantines from 140-trade failure window (2026-04-22). `pullback_reclaim×NEUTRAL` quarantined (n=115, 0% WR, avg -$1.29) via `SPOT_PULLBACK_RECLAIM_NEUTRAL_BLOCKED=true` in config → `spot_quality_block_reason()`. `pullback_reclaim×CHOP` quarantined (n=22, 0% WR) via `SPOT_PULLBACK_RECLAIM_CHOP_BLOCKED=true`. Taker fallback disabled globally (`SPOT_TAKER_FALLBACK_ENABLED=false`) — 113 taker trades all losers; if maker order unfills, cancel and skip. Stop tighten multipliers wired into `_compute_stop_pct()`: NEUTRAL 0.92×, CHOP 0.88×, pullback_reclaim 0.90×. Lane-specific kill switch: `runtime/spot_kill_switch.py` (KS10a: 4 consecutive losses → HALT; KS10b: daily PnL ≤ -2% of account → HALT); checked in `v10_runner._attempt_entry()` before spot entry. Calibrator lineage fixed: joins on `ml_feature_snapshots.trade_id` not fuzzy timestamp. Attribution ON CONFLICT fixed: 4-key UNIQUE (signal_name, regime, strategy, source). ML inference symbol routing: `model_store._get_pair_key()` strips -USD/-USDC suffix. Failure window backfilled: 140 trades reconstructed into ml_feature_snapshots + trade_attribution. Governance docs: `STOP_MATRIX.md`, `SCANNER_PRECISION_REPORT.md`, `RUNTIME_INVARIANTS.md`, `PROFIT_GOVERNANCE.md`, `DEPLOYMENT_STATE_MACHINE.md`, `MATRIX_DECISION_UNIVERSE.md`. 535 proof tests pass. Deployment verdict: READY_FOR_TINY_LIVE.
- **Spot scalp lane v17.4:** 8-symbol 24/7 spot scalp (BTC/ETH/SOL/XRP/LTC/DOGE/ADA/LINK). Universe governed by `SPOT_SYMBOLS` config. Multi-timeframe state (5m/30m/4h/1d) computed by `runtime/spot_momentum.py` — derivative stack z/v/a, normalized KST+Ichimoku+SuperTrend 2-of-3 structural confirms. Regime classifier (TREND/NEUTRAL/CHOP) in `runtime/spot_regime.py` from 30m+4h frame scores. Setup families: `impulse_continuation`, `pullback_reclaim`, `compression_breakout`. Final score = 0.60×composite + 0.40×derivative (config: `SPOT_SCALP_SCORE_WEIGHT_COMPOSITE/DERIVATIVE`). Risk-from-stop sizing: per-symbol `stop_floor_pct`/`stop_cap_pct`/`risk_fraction`/`allocation_cap_pct`/`spread_cap_pct`/`depth_min_usd`/`cooldown_min` in `SPOT_SCALP_SYMBOL_CONFIG`. Maker-first entry/exit via `runtime/spot_execution_policy.py` (post-only limit, wait `SPOT_MAKER_WAIT_SECONDS=6`, fallback taker). Execution route persisted to `open_positions.execution_route`. Dedicated `spot_scalp_scan()` (60s, `SPOT_SCALP_SCAN_SECONDS`) and `spot_exit_monitor()` (5s, `SPOT_EXIT_POLL_SECONDS`) schedules in v10_runner. New exits: stagnation + thesis-decay (min hold `SPOT_THESIS_MIN_HOLD_MINS=8`, score floor `SPOT_THESIS_MIN_SCORE=48`). EOD flatten disabled by default (`SPOT_EOD_FLATTEN_ENABLED=false`) for 24/7 operation. Session gating removed. `SPOT_MAX_DEPLOYED_PCT` raised to 0.50. New structured columns persisted to `scan_candidates` and `open_positions`: `spot_regime`, `setup_family`, `tf_5m/30m/4h/1d_state`, `structural_confirms`, `execution_route`, `cooldown_until`, `microstructure_veto`, `stop_model_version`, `target_model_version`, `target_r`, `trail_arm_r`, `risk_dollars`, `entry_fee_usd`, `exit_reason`. Spot balance surfaces emit generic `held_usd_by_symbol` keys. 474 proof tests pass.
...
- Economics gate EV tiers: A+=0.8%, A=0.4%, B=0.15%; stop_multiplier=3.0; spread gate=25bps; depth gate=$5K/side (changed v16.13 — old 1.6%/0.8%/0.3% values are wrong)
...
| v16.13 | 2026-04-17 | Live universe expansion + gate loosening: (1) AUTONOMOUS_LIVE_PERP_SYMBOLS expanded to all 4 core symbols; (2) SPOT_LANE_ACTIVE default true + spot auto-entry wired into _attempt_entry (8% of balance for LONG BTC/ETH); (3) max_live_perps raised 1→3 in perps_engine; (4) all cooldowns removed from v10_runner (no re-entry lockout after close); (5) _MAX_SINGLE_POSITION_USD $100 hardcap replaced with _MAX_SINGLE_POSITION_PCT=0.12 (scales with account); (6) Kelly source includes live_v10 trades; (7) leverage fallback uses composite_score when ML is untrained; (8) economics gate tiers reverted to pre-v13 values (A+ 0.8%, A 0.4%, B 0.15%) + ranging 1.67× multiplier removed; (9) Tier-2 composite threshold 58→55, Tier-1 floor 50→45; (10) WAE explosion LONG: dual MACD AND→OR; (11) supertrend_cross_long: KST+MACD AND→OR; (12) ranging_mr VWAP threshold ±0.30→±0.20; (13) entry_priors WR clip 0.70→0.80; 346 proof tests |
| v16.14c | 2026-04-18 | Fix scan liveness false alarm: health_check._check_scan_liveness() now uses lane_runtime_state.last_heartbeat_at (updated every 1m) as primary; v10_runner writes scan heartbeat before 0-candidate early return so system_events doesn't go stale during quiet markets; get_last_scan_age() returns min(heartbeat_age, log_age) |
| v16.14 | 2026-04-18 | Dashboard live-mode data isolation: added LIVE_START_DATE="2026-04-15" + get_effective_launch_date() to db.py; decision_quality.py + deep_analysis.py trade_attribution queries now filter AND paper=? (was leaking all 18,673 paper attribution rows into live-mode views); balance.py _unrealized_pnl() fixed hardcoded paper=1 → mode-aware; execution.py uses get_effective_launch_date(); status_hero/edge_quality/execution_quality captions show live start date in live mode; 8 stale paper open_positions purged from DB; 346 proof tests |
| v16.16 | 2026-04-19 | Truth-control pass: master SYSTEM SETTINGS trading-control panel, DB-first scanner/control-plane truth, shared manual-scan tradeability across rows/preview/spot controls, paper spot balance summary from DB, dashboard DB alias unification, forecast heartbeat freshness + richer runtime-state truth, 408 proof tests |
| v17.0 | 2026-04-19 | Dashboard control-tower architecture: 5-tab layout (CONTROL TOWER/CRYPTO/FORECAST/PERFORMANCE LAB/ENGINEERING CONSOLE). CONTROL TOWER = default operator surface. CRYPTO = unified spot+perp+manual-trade workflow. FORECAST = dedicated page with stale-heartbeat truth. PERFORMANCE LAB = strategy/attribution analysis. ENGINEERING CONSOLE = raw config + archived MES. New page composition layer (dashboard/widgets/pages/), data orchestrators (control_tower/crypto_dashboard/engineering_console), 431 proof tests |
| v17.1 | 2026-04-19 | Stabilization baseline: persisted live account-size truth (`system_runtime_state.account_size_live` + `runtime/live_account.py`), crypto lane runtime row now writes real broker/buying-power state, live risk floor aligned with kill-switch baseline, bare BTC candle routing fixed, go_live waits for connected crypto lane + non-zero buying power, missing tax-tracker noise suppressed |
| v17.2 | 2026-04-20 | US equity swing-trading lane: `execution/ibkr_stock_broker.py` (clientId=4, bracket orders), `scheduler/stock_runner.py` (standalone 8-signal tower, 30m schedule, market-hours gate), `dashboard/data/stocks.py`, `dashboard/widgets/stocks/stocks_dashboard.py` (4 subtabs), STOCKS tab added to `dashboard/app.py` between CRYPTO and FORECAST, `STOCKS_LANE_ACTIVE` + `STOCK_UNIVERSE` + `STOCKS_MAX_POSITIONS` + `STOCKS_RISK_PCT` + `STOCKS_MAX_POSITION_PCT` added to `config.py`, stocks lane upserted to `lane_runtime_state` and started as daemon thread in `main.py` |
| v17.3 | 2026-04-20 | Spot day-trading exits: `check_spot_targets()` (3R fixed target, called every 30s from exit monitor), `check_spot_eod_close()` (flatten all spot by 15:45 ET), `open_spot()` now persists real `target_price` to DB (was 0.0), `SPOT_TARGET_R=3.0` + `SPOT_EOD_CLOSE_TIME="15:45"` added to config. Fee math proved: 1.2% round-trip + 3R → break-even WR ~35%, economics gate score floor 74 ensures EV-positive trades only. 8 new proof tests SDT-01..08, 457 total passed. |
| v17.5 | 2026-04-27 | Self-calibrating spot edge conditions: stripped hardcoded edge_conditions from all 8 SPOT_SYMBOL_STRATEGY_OVERRIDES (n=12–47 backtest data, blocking 100% of entries); added learning/spot_edge_calibrator.py (derives conditions from >= 30 real closed trades, writes to spot_edge_conditions table); _load_db_conditions() in spot_strategy.py reads DB conditions at runtime (open gate until calibrated); nightly_audit now runs calibration; 3 stale proof tests updated to assert open-gate invariant; 422 proof tests pass. |
| v18.8 | 2026-04-28 | Aggressive gate loosening for high-volume trading: SPOT_TARGET_R_BY_REGIME TREND 2→4, NEUTRAL 1.5→3, CHOP 1.2→3; all 8 symbol score_floors halved (~24-25); SPOT_REGIME_SCORE_FLOORS TREND/NEUTRAL 48→24, CHOP 56→28; all depth_min_usd→0; all min_confirm_count→0; score_floor_for_symbol hard min 45→35; LTC/DOGE/ADA/LINK synthetic candidate injection into spot_scalp_scan path (perp scanner never covers them); risk/economics_gate.py _MIN_NEAR_DEPTH_USD 5K→1K; 3 proof tests updated; 512 proof tests pass. |
| v18.9 | 2026-04-28 | Bypass perp econ gate + execution universe gate for synthetic spot candidates: raise ImportError inside perp econ gate skips it for spot_only_synthetic=True; execution universe gate condition adds `and not candidate.get("spot_only_synthetic")` so LTC/DOGE/ADA/LINK pass through to spot_economics_gate.py with real build_spot_state() data; vol_usd=500M placeholder bypasses volume floor; 512 proof tests pass. |
| v18.10 | 2026-04-28 | All 8 spot symbols now trading: ADA spot entered $66, DOGE spot entered $41 in first scan after fix. LTC/LINK blocked only by spot_deployment_cap_exceeded (50% cap) — correct behavior when 7 positions already open. Full pipeline: synthetic inject → tier score → skip perp econ gate → skip research_only_block → tradeability → spot_economics_gate → spot execution. |
| v18.11 | 2026-04-28 | Spot lane surgery (Codex+Claude): (1) truth layer — spot close now writes ml_feature_snapshots + trade_attribution with canonical lineage (candidate_id, route_type, setup_family, spot_regime, tv_*); signal_stats ON CONFLICT 4-key fix; calibrator joins on trade_id not fuzzy timestamp; ML inference uses symbol-aware model key; (2) backfill — scripts/backfill_spot_failure_window.py reconstructed 140 live spot closes into learning tables; (3) governance — pullback_reclaim×NEUTRAL quarantined (n=115, 0% WR); pullback_reclaim×CHOP quarantined (n=22, 0% WR); taker_fallback disabled (SPOT_TAKER_FALLBACK_ENABLED=false); stop tighten multipliers wired into _compute_stop_pct (NEUTRAL 0.92×, CHOP 0.88×, pullback_reclaim 0.90×); (4) kill switch — runtime/spot_kill_switch.py (KS10a: 4 consecutive losses, KS10b: -2% daily) checked in v10_runner before spot entry; (5) 14 new proof tests (SG-01..14), 535 total; (6) governance docs: STOP_MATRIX.md, SCANNER_PRECISION_REPORT.md, RUNTIME_INVARIANTS.md, PROFIT_GOVERNANCE.md, DEPLOYMENT_STATE_MACHINE.md, MATRIX_DECISION_UNIVERSE.md. |
| v18.13 | 2026-04-29 | Operator unlocked full trading: `SPOT_PULLBACK_RECLAIM_NEUTRAL_BLOCKED` and `SPOT_PULLBACK_RECLAIM_CHOP_BLOCKED` defaults changed `true`→`false` (quarantine mechanism intact, flags off by default; re-enable via env var). Kill switch reset. Stale ADA/LINK/ETH positions deleted from DB (owner managing at Coinbase directly). SSP-04 test updated to use explicit patch instead of relying on default. 535 proof tests pass. |
| v18.12 | 2026-04-29 | Kill switch schema fix + live pipeline acceptance test: (1) `runtime/spot_kill_switch.py` `_is_halted()` and `reset_spot_kill_switch()` rewired to use `system_events` as truth (old code targeted non-existent `lane_runtime_state.spot_kill_switch_active` column → silent failures); halt state now derived from most-recent `spot_kill_switch` source event level; (2) `scripts/acceptance_test_spot_pipeline.py` — one-shot live BTC-USD round-trip acceptance test; (3) live verified 2026-04-29: BTC bought @ $76,465 (maker_first, 3.4s fill), closed @ $76,486 (maker_first, 5.4s fill), pnl=-$0.24; all learning pipeline checks passed: ml_feature_snapshots row 502 written (reconstructed=0), trade_attribution row 27118 written (candidate_id=999999), lineage fields threaded end-to-end (setup_family=impulse_continuation, spot_regime=TREND, route_type=maker_first), signal_stats 4-key ON CONFLICT upsert confirmed; kill switch correctly armable and queryable via system_events; 535 proof tests pass. |
| v17.4 | 2026-04-22 | 8-symbol 24/7 crypto spot scalp lane: expanded Coinbase spot universe to BTC/ETH/SOL/XRP/LTC/DOGE/ADA/LINK; multi-timeframe state engine (runtime/spot_momentum.py: 5m/30m/4h/1d, derivative stack z/v/a, KST+Ichimoku+SuperTrend 2-of-3 confirms); regime classifier TREND/NEUTRAL/CHOP (runtime/spot_regime.py); setup families impulse_continuation/pullback_reclaim/compression_breakout; blended composite+derivative scoring; risk-from-stop sizing with per-symbol SPOT_SCALP_SYMBOL_CONFIG; maker-first entry/exit routing (runtime/spot_execution_policy.py); dedicated spot_scalp_scan (60s) + spot_exit_monitor (5s) schedules; stagnation+thesis-decay exits; EOD flatten disabled by default; structured spot scalp truth persisted to DB; broker-truth dashboard/manual alignment; 474 proof tests. |
