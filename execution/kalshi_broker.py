...
class KalshiBroker:
    ...
    def _apply_exit_fill(
        ...
        *,
        ...
        if exit_price > 0:
            # A NO holder gains when the contract price falls, so the legs are
            # complements: PnL = entry - exit. Reverting 174d23d, which deleted
            # this branch and silently reported every NO exit with long-YES sign.
            if held_side == "NO":
