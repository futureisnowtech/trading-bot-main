...
KNOWN_STRATEGIES = {
    ...
    "v10_perp",  # v10 two-tower signal engine (current live strategy)
}
...
def check_duplicate_closes(conn, days):
    ...
    if rows:
        ...
        for r in rows[:20]:
            print(
                f"   {r['ts'][:16]} {r['symbol']:12} qty={r['qty']:.4f}  "
...
def check_orphaned_open_positions(conn):
    ...
    if orphans:
        ...
        for sym, strat, ts_entry, ts_close in orphans:
            print(
                f"   {sym:12} {strat:25} entry={ts_entry[:16]}  close={ts_close[:16]}"
            )
...
def check_duplicate_buys(conn, days):
    ...
    if rows:
        ...
        for r in rows[:20]:
            print(
                f"   {r['ts'][:16]} {r['symbol']:12} {r['strategy']:25} qty={r['qty']:.4f}"
            )
    else:
...
def check_pnl_reconciliation(conn, days):
    ...
    print(
        f"   Closed trades:  {r['closed_trades']}  ({r['wins']}W / {r['losses']}L, {wr:.1%} WR)"
    )
...
def check_outlier_trades(conn, days):
    ...
    if outliers:
        ...
        for r in outliers:
            print(
                f"   id={r['id']} {r['ts'][:16]} {r['symbol']:12} "
                f"P&L={r['pnl_usd']:+.4f}  qty={r['qty']:.4f} @ ${r['price']:.4f}"
            )
    else:
        print(
            f"✅  No outlier trades (avg |P&L| = ${avg_abs:.4f}, "
...
def check_id_ordering(conn, days):
    ...
    for i in range(1, len(rows)):
        if rows[i]["ts"] < rows[i - 1]["ts"]:
            issues.append(
                (rows[i - 1]["id"], rows[i - 1]["ts"], rows[i]["id"], rows[i]["ts"])
            )
    if issues:
        print(
...
def check_unknown_strategies(conn, days):
    ...
    rows = cur.fetchall()
    unknown = [
        (r["strategy"], r["cnt"]) for r in rows if r["strategy"] not in KNOWN_STRATEGIES
    ]
    if unknown:
...
def check_file_permissions():
    ...
    for path, recommended, desc in targets:
        ...
        flags = []
        if world_read:
            flags.append("world-readable")
        if world_write:
            flags.append("world-writable")
        if group_write:
            flags.append("group-writable")
...
def check_db_tampering(conn, days):
    ...
    if rows:
        ...
        for r in rows[:10]:
            print(
                f"   id={r['id']} {r['ts'][:16]} {r['symbol']:12} fee={r['fee_usd']:.6f}"
            )
        issues += len(rows)
    else:
    ...
    if magnitude_hits:
        ...
        for r in magnitude_hits[:10]:
            print(
                f"   id={r['id']} {r['ts'][:16]} {r['symbol']:12} pnl={r['pnl_usd']:+.4f}"
            )
...
def check_off_hours_trades(conn, days):
    ...
    if rows:
        ...
        for r in rows[:15]:
            print(
                f"   {r['ts'][:19]} UTC  {r['symbol']:12} {r['action']:8} "
                f"{r['strategy']:25} pnl={r['pnl_usd']:+.4f}"
            )
        print(
            "   → Investigate: did the bot bypass the time gate, or were these injected?"
        )
    else:
...
def check_log_anomalies():
    ...
    log_patterns = [
        (
            re.compile(r"(?i)(401|unauthorized|invalid.*api.*key|api.*key.*invalid)"),
            "AUTH FAILURE",
            "CRITICAL",
        ),
        (re.compile(r"(?i)(403|forbidden|permission.*denied)"), "FORBIDDEN", "HIGH"),
        (
            re.compile(r"(?i)(credential|password|secret).*(error|fail|invalid)"),
            "CRED ERROR",
            "HIGH",
        ),
        (re.compile(r"(?i)rate.?limit"), "RATE LIMIT", "MEDIUM"),
        (
            re.compile(r"(?i)(connection.?refused|timeout|ssl.*error)"),
            "CONN ERROR",
            "LOW",
        ),
    ]
        ...
        for pattern, label, severity in log_patterns:
            if pattern.search(line):
                counts[label] = counts.get(
                    label, {"count": 0, "severity": severity, "examples": []}
                )
    ...
    exception_lines = [
        l for l in tail if "Traceback" in l or "Exception" in l or "Error:" in l
    ]
...
def check_credential_backup():
    ...
    latest = backups[0]
    age_days = (
        datetime.now(timezone.utc).timestamp() - os.path.getmtime(latest)
    ) / 86400
    if age_days > 7:
    ...
    print(
        f"✅  Credential backup exists ({os.path.basename(latest)}, {age_days:.1f}d old, "
...
def main():
    parser = argparse.ArgumentParser(
        description="Trade DB health check + security audit"
    )
    parser.add_argument(
        "--days", type=int, default=1, help="Trade lookback window in days (default: 1)"
    )
    parser.add_argument(
        "--no-sec",
        action="store_true",
