...
def check_exits(
    ...
    if _entry_setup in _MOMENTUM_SETUPS:
        _min_hold_secs = (
            7200  # 2 hours for WAE/squeeze momentum (single-bar signals need room)
        )
    ...
    if not scale_33_done and r_gained >= 2.0:
        return ExitDecision(
            True, 2, "scale_out_33", f"2R reached (r={r_gained:.2f}) — close 33%", 0.33
        )
    ...
    if scale_33_done and not scale_66_done and r_gained >= 3.5:
        return ExitDecision(
            ...
            2,
            "scale_out_66",
            f"3.5R reached (r={r_gained:.2f}) — close next 33%",
            0.33,
        )
    ...
    if not trailing_active:
        if is_long and current_price >= entry + atr:
            ...
            return ExitDecision(
                ...
                0,
                "trailing_activated",
                f"Trailing stop activated at {new_trail:.4f}",
                0.0,
            )
