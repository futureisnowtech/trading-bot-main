...
def symbol_class_quality(conn: sqlite3.Connection, days: int) -> list[dict]:
    """Win rate by execution tier: core vs research_only."""
    cut = _cutoff(days)
    # Use entry_block_reason to infer tier: research_only_block = research_only
    rows = _query(
        conn,
        """
        SELECT
            CASE
                WHEN sc.decision = 'entered' THEN 'core'
                WHEN sc.decision = 'research_only_block' THEN 'research_only'
                ELSE sc.decision
            END AS tier,
            COUNT(*) AS n,
            SUM(CASE WHEN co.hit_1r=1 AND co.hit_stop=0 THEN 1 ELSE 0 END) AS wins,
    ...
    return [
        {
            ...
            "win_rate_pct": round((r.get("win_rate") or 0) * 100, 1),
            "avg_mfe_pct": round(r.get("avg_mfe") or 0, 3),
            "avg_mae_pct": round(r.get("avg_mae") or 0, 3),
        }
        for r in rows
    ]
...
def integrity_snapshot(conn: sqlite3.Connection, days: int) -> dict:
    """Integrity tier breakdown and duplicate close detection."""
    cut = _cutoff(days)

    tier_rows = _query(
        conn,
        """
        SELECT integrity_tier, COUNT(*) AS n
        FROM trade_integrity
        WHERE datetime(replace(substr(created_at,1,19),'T',' ')) >= datetime(?)
        GROUP BY integrity_tier
        ORDER BY n DESC
        """,
        (cut,),
    )
    ...
    dup_rows = _query(
        conn,
        """
        SELECT close_order_id, COUNT(*) AS n
        FROM trade_integrity
        WHERE datetime(replace(substr(created_at,1,19),'T',' ')) >= datetime(?)
    ...
    tiers = {r["integrity_tier"]: r["n"] for r in tier_rows}
    ...
    return {
        ...
        "duplicate_close_ids": len(dup_rows),
        "duplicate_examples": [r["close_order_id"] for r in dup_rows[:3]],
    }
...
def _print_report(report: dict) -> None:
    ...
    for tier, n in sorted(ig["tiers"].items()):
        print(f"     {tier:<16}: {n}")
    if ig["duplicate_close_ids"]:
        print(f"   ⚠  Duplicate close IDs: {ig['duplicate_close_ids']}")
        for cid in ig["duplicate_examples"]:
            print(f"      {cid}")
    else:
        print("   No duplicate close IDs detected.")
